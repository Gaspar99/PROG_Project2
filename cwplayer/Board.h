...
public:
    // Constructor for the Board class which generates a std::map and its respective keys which represent coordinates on the board
...
private:
    // The board itself is a std::map with std::strings as keys representing coordinates followed by a char value representing the
    ...
    // Returns a vector of std::strings containing the coordinates of the cells to be modified.
    // If any of these coordinates is out of bounds, the corresponding position in the std::vector will be empty.
    // The std::vector is initialized with null characters. Built into addWords.
